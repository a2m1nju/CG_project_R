...
const float TAP_PER_SECOND = 10.0f; // 1초당 필요한 연타 횟수 (게이지 상승률)
...
void timer(int value)
{
	...
	//}
		...
		// 성공 조건: 4초 동안 필요한 총 횟수 (예: 4초 * 10회/초 = 40회)
		...
		if (!isDashing) {
			...
			if (!isMoving && !isDashing && !isFlying) {
				playerPos.y = restingY;
				playerStartPos.y = restingY;
			}
